...
func (s *BlogService) UpdatePost(ctx context.Context, req *connect.Request[blogv1.UpdatePostRequest]) (*connect.Response[blogv1.UpdatePostResponse], error) {
	...
	ifVersion, err := preconditionVersion(req.Msg.IfVersion)
	if err != nil {
		return nil, err
	}
	p, err = s.posts.UpdateWithTags(tctx, p.ID, req.Msg.GetTitle(), req.Msg.GetSlug(), req.Msg.GetBodyMarkdown(), catID, tagIDs, ifVersion)
	if err != nil {
		return nil, err
	}
	msg, err := s.toProto(tctx, p)
...
func (s *BlogService) DeletePost(ctx context.Context, req *connect.Request[blogv1.DeletePostRequest]) (*connect.Response[blogv1.DeletePostResponse], error) {
	...
	if delErr := s.posts.Delete(tctx, p.ID, ifVersion); delErr != nil {
		return nil, delErr
	}
...
func (s *BlogService) PublishPost(ctx context.Context, req *connect.Request[blogv1.PublishPostRequest]) (*connect.Response[blogv1.PublishPostResponse], error) {
	...
	p, err = s.posts.Publish(tctx, p.ID, ifVersion)
	if err != nil {
		return nil, err
	}
	msg, err := s.toProto(tctx, p)
...
func (s *BlogService) UnpublishPost(ctx context.Context, req *connect.Request[blogv1.UnpublishPostRequest]) (*connect.Response[blogv1.UnpublishPostResponse], error) {
	...
	p, err = s.posts.Unpublish(tctx, p.ID, ifVersion)
	if err != nil {
		return nil, err
	}
	msg, err := s.toProto(tctx, p)
...
func preconditionVersion(v *int32) (int, error) {
	if v == nil {
		return 0, nil
	}
	if *v <= 0 {
		return 0, validationErr("if_version", "if_version must be the post version returned by the API; omit the field for an unconditional write")
	}
	return int(*v), nil
}

func versionToProto(v int) int32 {
